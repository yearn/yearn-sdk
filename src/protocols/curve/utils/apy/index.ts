...
import { calculateApyPps } from "../../../yearn/assets/vault/apy";
import { Apy as VaultApy } from "../../../yearn/assets/vault/apy/common";
import { getPoolFromLpToken } from "../";
...
export async function calculateApy(lpToken: string, ctx: Context): Promise<Apy> {
  ...
  if (btcMatch) {
    priceOfBaseAsset = await getPrice(wbtcAddress);
  } else if (ethMatch) {
    priceOfBaseAsset = await getPrice(wethAddress);
  } else {
    priceOfBaseAsset = (await getPrice(firstUnderlyingCoinAddress)) || 1;
  }

  const priceOfCrv = await getPrice(crvAddress);
  ...
  const data = {
    baseApy: baseApy.toFixed(),
    currentBoost: currentBoost.toFixed(4),
    boostedApy: boostedApy.toFixed(),
    poolApy: poolApy.toFixed(8),
    totalApy: aggregateApy.toFixed(8)
  };
