...
var ContractService = /*#__PURE__*/function (_Service2) {
  ...
  function ContractService(address, chainId, ctx) {
    ...
    _this3.contract = new Contract(_this3.address, // @ts-ignore
          ...
          switch (_context2.prev = _context2.next) {
            ...
            case 4:
              ...
              Object.keys(balances).forEach(function (address) {
                ...
                balances[getAddress(address)] = copy.map(function (balance) {
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 11:
              ...
              if (earningsUsdc.gt(BigNumber.from(0)) && earningsUsdc.lt(BigNumber.from(1000000000))) {
          ...
          switch (_context4.prev = _context4.next) {
            case 0:
              sumTokenAmountsFromInteractions = function _sumTokenAmountsFromI(interactions, vaultAddress) {
                return interactions.filter(function (deposit) {
                  ...
                  return BigNumber.from(deposit.tokenAmount);
                    ...
                    while (1) {
                      switch (_context3.prev = _context3.next) {
                        case 0:
                          ...
                          positiveValues = BigNumber.from(vaultPosition.tokenAmount).add(withdrawalsTotal).add(sharesSentTotal);
                          negativeValues = depositsTotal.add(sharesReceivedTotal);
                          totalTokensEarned = BigNumber.from(0);
            ...
            case 5:
              ...
              vaultPositions = account.vaultPositions.map(function (vaultPositionContainer) {
                var shares = BigNumber.from(vaultPositionContainer.balanceShares);
                var pricePerShare = BigNumber.from(vaultPositionContainer.vault.latestUpdate.pricePerShare);
                var decimals = BigNumber.from(vaultPositionContainer.token.decimals);
                var tokenAmount = shares.mul(pricePerShare).div(BigNumber.from(10).pow(decimals));
                var result = {
              ...
              earnings = [];
              totalEarningsUsdc = BigNumber.from(0);
            ...
            case 15:
              ...
              totalEarningsUsdc = totalEarningsUsdc.add(BigNumber.from(vaultUserEarnings.amountUsdc));
        ...
        while (1) {
          switch (_context5.prev = _context5.next) {
            ...
            case 2:
              ...
              return _context5.abrupt("return", BigNumber.from(tokenUsdcPrice).mul(tokenAmount).div(BigNumber.from(10).pow(BigNumber.from(token.decimals))));
...
function calculateEarningsForVault(vault) {
  var pricePerShare = BigNumber.from(vault.latestUpdate.pricePerShare);
  var totalTokens = pricePerShare.mul(BigNumber.from(vault.sharesSupply));
  var earnedTokens = totalTokens.sub(BigNumber.from(vault.balanceTokens)).div(BigNumber.from(10).pow(BigNumber.from(vault.token.decimals)));
  return earnedTokens;
}
...
export { ApyService, LensService, OracleService, TokenReader, VaultReader, Yearn, ZapperService };
